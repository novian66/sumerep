...
if(isset($_POST['submit'])){
    ...
    $stmt4 = $pdo->prepare("INSERT INTO `check_done` (`categories_id`, `user_id`, `done`, `created` , `type`) VALUES ('$idcats', '$id_siswa', 'yes', current_timestamp() , 'puskesmas');");
        if($stmt4->execute()){
            $msg = 'Success!!';
            echo "<script type='text/javascript'>alert('$msg'); window.location.href='detail-kelas-puskesmas.php?sekolah=$id_sekolah&kelas=$id_kelas'</script>";
            }else{
                $msg = 'Error!!';
            echo "<script type='text/javascript'>alert('$msg'); window.location.href='detail-kelas-puskesmas.php?sekolah=$id_sekolah&kelas=$id_kelas'</script>";
            }
...
?>
<?=headersiswa('Screening',"$nama",$foto,'puskesmas')?>
